...
def convert_variant_df_to_msa(variant_df, parent_seq):
    """
    ...
    seq_ids = ['PARENT']
    for label, predicted_variant in variant_df[['Path', 'Variant']].values:
        seq = list(parent_seq)
...
def make_msa(file_to_align):
    msa_file = f'{file_to_align.replace(".fasta", "_msa.fa")}'
