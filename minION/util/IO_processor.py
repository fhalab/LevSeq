        ...
        if isinstance(variant, float):
            ...
            for var in variants:
                ...
                if new_nucleotide == "DEL":
                    ...
                    new_seq = (
                        new_seq[:adj_pos] + new_nucleotide + new_seq[adj_pos + 1 :]
                    )
...
class BarcodeProcessor:
    ...
    def filter_barcodes(self, filtered_fasta_file, front_range, reverse_range):
        ...
        filtered_records = [
            ...
            if (
                record.id.startswith(self.front_prefix)
                and min_front <= int(record.id[len(self.front_prefix) :]) <= max_front
            )
            or (
                record.id.startswith(self.reverse_prefix)
                and min_reverse
                <= int(record.id[len(self.reverse_prefix) :])
                <= max_reverse
            )
        ]
    ...
    def get_barcode_dict(self, demultiplex_folder: Path) -> dict:
        ...
        rbc_folders = get_rbc_barcode_folders(
            demultiplex_folder, prefix=self.reverse_prefix
        )
...
def find_folder(start_path, target_folder_name):
    ...
    raise Exception(
        f"{target_folder_name} folder does not exist. Please check if you have chosen the right name."
    )
...
def find_experiment_folder(
    experiment_name: str, minknow_path: Path = MINKNOW_PATH
) -> None:
    ...
    raise Exception(
        "Experiment folder does not exist. Please check if you have chosen the right experiment name."
    )
...
def filter_fastq_by_length(
    input_fastq: Path, output_fastq: Path, min_length: int, max_length: int
):
    """
...
def find_file(start_path: Path, prefix: str, extension: str) -> Path:
    ...
    raise Exception(
        f"No file with prefix '{prefix}' and extension '{extension}' was not found in {start_path}."
    )


def concatenate_fastq_files(
    path: Path, filename: str = "concatenated", prefix: str = "*", delete: bool = True
) -> str:
    """
...
def read_fasta_file(path: Path, score=False) -> dict:
    ...
    if score:
        ...
        for record in SeqIO.parse(path, "fastq"):
            ...
            sequences_and_scores["Quality-Score"].append(
...
def create_folder(
    ...
    if target_path is None:
        ...
        if not target_path.exists():
            raise Exception(
                "Target path does not exist. Please check if you have chosen the right path."
            )
...
def get_fbc_barcode_folders(rbc_barcode_folders: list, prefix="barcode") -> dict:
    ...
    if rbc_barcode_folders is None:
        raise Exception(
            "Reverse barcode folders are not given. Please check if you have chosen the right path."
        )
    ...
    if not any(fbc_folders.values()):
        raise Exception(
            f"Forward barcodes in {rbc_barcode_folders} do not exist. Please check the folders."
        )
...
def get_barcode_dict(
    demultiplex_folder: Path, front_prefix="barcode", reverse_prefix="barcode"
) -> dict:
    """
    Get a dictionary of folder paths, where reverse is the key and forward is stored in a list
...
def filter_fastq_by_length(
    result_folder, input_fastq: Path, min_length: int, max_length: int, ind: int
):
    """
    ...
    if input_fastq.is_dir():
        ...
        raise Exception(
            "Input file is not a file or a folder. Please check if you have chosen the right path."
        )
