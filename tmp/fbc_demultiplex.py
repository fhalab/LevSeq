...
barcode_kit = 'MY-CUSTOM-BARCODES-EVSEQ'
fbc_input = pass_pwd
...
# Create empty dataframe for storing forward counts
df_ct_fwd  = pd.DataFrame({'FBC':[], 'Aligned':[]})
for candidate_fw in candidates_fw:
    ...
    regex_ct = r'runid'
    f_fwd = open(fw, 'r')
    ct_fwd = len(re.findall(regex_ct,f_fwd.read()))

    # Append sequence count to dataframe
    data_temp = {'FBC':[fbc], 'Aligned':[ct_fwd]}
    df_temp = pd.DataFrame(data_temp)
    df_ct_fwd = pd.concat([df_temp,df_ct_fwd], axis=0)

fwd_list = df_ct_fwd.values.tolist() 
fwd_ct = [tuple(x) for x in fwd_list]

hbar = hv.Bars(fwd_ct)
hv.save(hbar, 'fwd.html')
'''
